...
│   └── md5.text        # MD5 去重记录（已废弃，改用 MySQL）
├── data/               # 示例数据
│   ├── 海绵宝宝.txt
│   ├── 精灵宝可梦.txt
│   ├── 猫和老鼠汤姆和杰瑞细节.txt
...
1. 注册/登录：首次使用需要注册账号
2. 创建对话：点击侧边栏的 "+" 按钮
3. 上传文件：点击侧边栏的 "📋" 图标进入知识库管理页面上传文件
4. 知识库管理：可查看/删除/共享自己的文件，查看公共知识库
5. 提问：在输入框中输入问题，按 Enter 发送
6. 查看历史：侧边栏会显示所有会话历史
7. 知识库隔离：RAG 检索时仅检索你上传的文件 + 公共文件
...
+ KnowledgeBaseService：知识库管理服务，支持上传、去重、共享、删除等操作。
...
| is_shared | string | 否 | 是否共享到公共知识库（"true"/"false"） |
...
说明：系统会自动计算文件 MD5 进行去重检查，重复内容会返回跳过提示。
...
+ 删除时：仅从 MySQL 删除文件记录（Milvus 中已有数据保留，避免影响其他用户已共享的检索）
