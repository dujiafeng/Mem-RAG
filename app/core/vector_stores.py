...
class VectorStoreService:
    ...
    def search_milvus(self, query, k=3, user_id: int = None):
        """底层手动搜索，绕过 LangChain Milvus 类的 Bug
           user_id: 如果提供，只搜索该用户的文件 + 公开文件
        """
        # 检查集合是否存在
        ...
        if user_id is not None:
            # 搜索用户自己的文件（user_id == X）或公开共享文件（is_shared == 1）
            filter_expr = f"user_id == {user_id} or is_shared == 1"
            logger.info(f"[Milvus] 用户隔离过滤: {filter_expr}")
        ...
        try:
            search_params = {
                ...
                "data": [query_vector],
                "limit": k,
                "output_fields": ["text", "filename", "user_id", "is_shared"]
            }
            if filter_expr:
            ...
            # 4. 将结果转为 LangChain 的 Document 格式
            docs = []
            for hit in res[0]:
                doc = Document(
    ...
    def get_retriever(self, user_id: int = None):
        ...
        class CustomMilvusRetriever(BaseRetriever):
            ...
            def _get_relevant_documents(self, query: str) -> List[Document]:
                return self.vector_service.search_milvus(query, k=self.k, user_id=self.user_id)

        dense_retriever = CustomMilvusRetriever(vector_service=self, k=config.SIMILARITY_THRESHOLD, user_id=user_id)
    ...
    def hybrid_search_workflow(self, query, user_id: int = None):
        """
        混合搜索工作流
        :param query: 查询语句
        :param user_id: 用户 ID，用于知识库隔离
        ...
        retriever = self.get_retriever(user_id=user_id)
