...
class RagService(object):
    ...
    def __get_chain(self):
        ...
        def retrieve_context(input_data: dict, run_config: RunnableConfig = None):
            query = input_data["input"]
            # 从 config 中提取 user_id（由 API 层传入）
            user_id = None
            if run_config and "configurable" in run_config:
                user_id = run_config["configurable"].get("user_id")
                if user_id is not None:
                    logger.info(f"[RAG] 用户隔离检索: user_id={user_id}")

            # 调用 VectorStoreService 的混合搜索
            status_messages, docs = self.vector_service.hybrid_search_workflow(query, user_id=user_id)
