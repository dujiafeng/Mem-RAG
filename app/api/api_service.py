...
from fastapi import FastAPI, Response, HTTPException, Depends, Cookie, Body, UploadFile, File, Form
...
from app.models.models import Base, User, ChatSession, ChatMessage, KnowledgeFile
...
async def get_current_user(session_id: str = Cookie(None), db: AsyncSession = Depends(get_db)) -> User:
    """从 Cookie 获取当前登录用户"""
    if not session_id:
...
async def login(username: str, password: str, response: Response, db: AsyncSession = Depends(get_db)):
    ...
    await db.commit()
    response.set_cookie(key="session_id", value=new_cookie, path="/", samesite="lax")  # 跨域建议
    return {"status": "success", "message": "登录成功", "data": {"username": user.username}}


@app.get("/auth/me")
async def check_login(session_id: str = Cookie(None), db: AsyncSession = Depends(get_db)):
    """检查当前登录状态"""
    if not session_id:
        raise HTTPException(status_code=401, detail="未登录")
    res = await db.execute(select(User).where(User.last_cookie == session_id))
...
async def chat_stream(session_uuid: str = Body(..., embed=True), input_text: str = Body(..., embed=True),
    ...
    async def event_generator():
        ...
        with redirect_stdout(stdout_capture):
            # 传递 user_id 到 RAG 链的 config 中，实现知识库隔离
            chain_config = {"configurable": {"session_id": session_uuid, "user_id": user.id}}
...
async def list_knowledge_files(
    ...
    """获取当前用户的知识库文件列表"""
    user = await get_current_user(session_id, db)
    files = kb_service.get_user_files(user.id)
...
async def list_shared_files():
    """获取所有公开共享的知识库文件"""
    files = kb_service.get_shared_files()
    return {"status": "success", "data": files}
...
async def delete_knowledge_file(
    ...
    """删除知识库文件"""
    user = await get_current_user(session_id, db)
    success, error = kb_service.delete_file(file_id, user.id)
    if not success:
        raise HTTPException(status_code=400, detail=error)
    return {"status": "success", "message": "文件已删除"}


@app.post("/kb/files/{file_id}/share")
async def toggle_share_knowledge_file(
    file_id: int,
    session_id: str = Cookie(None),
    db: AsyncSession = Depends(get_db)
):
    """切换知识库文件的共享状态"""
    user = await get_current_user(session_id, db)
    result, error = kb_service.toggle_share(file_id, user.id)
    if error:
        raise HTTPException(status_code=400, detail=error)
    return {"status": "success", "data": result}


@app.get("/kb/files/{file_id}/preview")
async def preview_knowledge_file(
    ...
    """预览知识库文件内容"""
    user = await get_current_user(session_id, db)
    result, error = kb_service.get_file_preview(file_id, user.id)
    if error:
        raise HTTPException(status_code=400, detail=error)
    return {"status": "success", "data": result}
